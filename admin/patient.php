...
<body>
    ...
    <div class="container">
        ...
        <div class="dash-body">
            <table border="0" width="100%" style=" border-spacing: 0;margin:0;padding:0;margin-top:65px; ">
                <tr >
                    <td width="13%">
...
?>
    ...
    if($_GET){
        ...
        // Get booking history
        $bookingHistory = $database->query("SELECT * FROM booking WHERE EMAIL='$email' ORDER BY DATE DESC");
                        ...
                        <table width="80%" class="sub-table scrolldown add-doc-form-container" border="0">
                            ...
                            <tr>
                                <td class="label-td" colspan="2">
                                    <label for="name" class="form-label">Patient ID: P-'.$id.'</label>
                                </td>
                            </tr>
                            <tr>
                                <td class="label-td" colspan="2">
                            ...
                            <tr>
                                <td class="label-td" colspan="2">
                                    <label for="spec" class="form-label">Address: '.$address.'</label>
                                </td>
                            </tr>
                            <tr>
                                <td class="label-td" colspan="2">
        ...
        if($bookingHistory->num_rows > 0) {
            while($historyRow = $bookingHistory->fetch_assoc()) {
                // Use dname from booking table if available, otherwise get from doctor table
                $doctorName = !empty($historyRow["dname"]) ? $historyRow["dname"] : getDoctorName($database, $historyRow["did"]);
                
                echo '<tr>
                        <td colspan="2">
                            <div style="padding: 10px;background-color: #f9f9f9;margin: 5px;border-radius: 5px;">
                                <p style="margin: 0;"><strong>Date:</strong> '.$historyRow["DATE"].'</p>
                                <p style="margin: 0;"><strong>Time:</strong> '.$historyRow["time_slot"].'</p>
                                <p style="margin: 0;"><strong>Doctor:</strong> '.$doctorName.'</p>
                                <p style="margin: 0;"><strong>Reason:</strong> '.($historyRow["dscrptn"] ?: 'Not specified').'</p>
                                <p style="margin: 0;"><strong>Status:</strong> '.$historyRow["status"].'</p>
                            </div>
                        </td>
                    </tr>';
            }
            ...
            echo '<tr>
                    <td colspan="2">
                        <p style="text-align: center;">No appointment history found.</p>
                    </td>
                </tr>';
        }
    ...
    // Update the getDoctorName function to handle NULL values
    function getDoctorName($database, $docId) {
        if ($docId === NULL) {
